...
build_dnstt() {
    ...
    if command -v go &>/dev/null; then
        echo -e "${CYAN}Building dnstt-server from source with Go...${NC}"
        rm -rf /tmp/dnstt-build
        mkdir -p /tmp/dnstt-build
        cd /tmp/dnstt-build

        git clone https://www.bamsoftware.com/git/dnstt.git 2>/dev/null || \
        git clone https://github.com/Mygod/dnstt.git 2>/dev/null

        if [[ -d "dnstt/dnstt-server" ]]; then
            cd dnstt/dnstt-server
            go build -o "$DNSTT_BIN" 2>/dev/null
        fi
        cd "$INSTALL_DIR"
        rm -rf /tmp/dnstt-build
    fi
    ...
    if [[ ! -f "$DNSTT_BIN" ]] || [[ ! -x "$DNSTT_BIN" ]]; then
        ...
        wget -q https://raw.githubusercontent.com/ASHANTENNA/VPNScript/main/dnstt-server -O "$DNSTT_BIN"
    fi

    chmod +x "$DNSTT_BIN"

    # Verify binary works
    if ! "$DNSTT_BIN" -h 2>&1 | grep -qi "usage\|gen-key\|privkey" ; then
        echo -e "${RED}WARNING: dnstt-server binary may not be functional.${NC}"
        echo -e "${RED}You may need to build it manually with Go.${NC}"
...
generate_keys() {
    ...
    # Keys are generated by: dnstt-server -gen-key
    if [[ -f "$PRIVKEY_FILE" ]] && [[ -f "$PUBKEY_FILE" ]]; then
        echo -e "${YELLOW}Existing keys found.${NC}"
    ...
    "$DNSTT_BIN" -gen-key -privkey-file "$PRIVKEY_FILE" -pubkey-file "$PUBKEY_FILE" 2>&1

    if [[ ! -f "$PRIVKEY_FILE" ]] || [[ ! -f "$PUBKEY_FILE" ]]; then
        echo -e "${RED}Key generation failed!${NC}"
        echo -e "${RED}Make sure dnstt-server binary is working correctly.${NC}"
    ...
    echo -e "${CYAN}Private key saved to: $PRIVKEY_FILE${NC}"
    echo -e "${CYAN}Public key saved to:  $PUBKEY_FILE${NC}"
    echo -e "\n${WHITE}=== PUBLIC KEY (give this to clients) ===${NC}"
    echo -e "${GREEN}$(cat "$PUBKEY_FILE")${NC}"
    echo -e "${WHITE}=========================================${NC}"
    echo
    echo -e "${YELLOW}IMPORTANT: Clients need this public key to connect.${NC}"
    echo -e "${YELLOW}Save it now! You can also view it later with: cat $PUBKEY_FILE${NC}"
...
setup_dnstt_service() {
    ...
    # This is the core of SlowDNS: SSH over DNS tunnel
...
[Service]
Type=simple
ExecStart=$DNSTT_BIN -udp :$DNSTT_PORT -mtu $MTU_SIZE -privkey-file $PRIVKEY_FILE $NAMESERVER 127.0.0.1:$SSH_PORT
Restart=always
RestartSec=3
LimitNOFILE=65535
...
EXPIRY
    ...
    if systemctl is-active --quiet ai-slowdns-tz.service; then
        ...
        echo -e "${RED}DNSTT server failed to start. Checking logs...${NC}"
        journalctl -u ai-slowdns-tz.service --no-pager -n 20
    fi
